import { collapse, expand, Tree, Tree7 } from "./trees.ts";
...
function drawTrees(svg: HTMLElement, trees: Tree[], rerender: () => void) {
  ...
  svg.setAttributeNS(null, "viewBox", `${-pad} ${-pad} ${width + pad * 2} ${height + pad * 2}`);
  svg.setAttributeNS(null, "width", `${width + pad * 2}`);
  svg.setAttributeNS(null, "height", `${height + pad * 2}`);

  function drawTree(tree: Tree, yPos: number): number {
    if (tree.length === 0) {
      const center = xPos;
      xPos += nodeSize + nodeSpacing;
      const node = drawNode(center, yPos);
    ...
    const left = drawTree(tree[0], yPos + rowSize);
    const right = drawTree(tree[1], yPos + rowSize);
    const center = (left + right) / 2;
    drawLine(center, yPos, left, yPos + rowSize);
    drawLine(center, yPos, right, yPos + rowSize);
    const node = drawNode(center, yPos);
  ...
  function drawNode(x: number, y: number): SVGCircleElement {
